...
import {findAllInstitution, deleteInstitution} from "@/app/admin/api/institution"
...
const compHandle = reactive({
    ...
    getTableData() {
        ...
        findAllInstitution().then((res) => {
            compData.tableData = res.data || []
            compData.allData = res.data || []
            compHandle.filterArr = []
            if (res.data && res.data.length > 0) {
                for (let item of res.data) {
                    if (!compHandle.filterArr.find(i => i.label === item.PGroupName) && item.PGroupName) {
    ...
    dels() {
        ...
        deleteModalRef.value.openDeleteModal('确认要删除所选机构吗？',function deleteFun() {
            deleteItem(ids)
    ...
    search() {
        let props = ['PGroupName', 'GroupName', 'GroupCode', 'DT']
        compData.tableData  = Search(compData.searchForm.keyWord, props, deepCopy(compData.allData))
    },
    exportData() {
        ExportTable(compData.allData, compData.columns, '管理机构')
    },
})
