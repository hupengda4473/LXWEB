...
import {findAllArea, deleteArea} from "@/app/admin/api/area"
...
import {deleteMenus} from "@/app/admin/api/menu"
...
const compHandle = reactive({
        ...
        findAllArea().then((res) => {
            ...
            if (res.data && res.data.length > 0) {
                for (let item of res.data) {
                    if (!compHandle.filterArr.find(i => i.label === item.PAreaName) && item.PAreaName) {
                        compHandle.filterArr.push({
                            label: item.PAreaName,
                            value: item.PID
                        })
                    }
                }
    ...
    dels() {
        ...
        deleteModalRef.value.openDeleteModal('确认要删除所选区域吗？',function deleteFun() {
            deleteAreaData(ids)
    ...
    search() {
        let props = ['AreaCode', 'AreaName', 'PAreaName', 'DT']
        compData.tableData  = Search(compData.searchForm.keyWord, props, deepCopy(compData.allData))
    },
    exportData() {
        ExportTable(compData.allData, compData.columns, '行政区域')
    },
})
...
//删除区域
const deleteAreaData = (ids: string | number) => {
    compData.loading = true
    deleteArea(ids).then(
        res =>{
            if (res.data.Code === 0){
