...
import {deletePersonnel, getAllPersonnel} from '@/app/admin/api/personnel'
...
const compHandle = reactive({
        ...
        getAllPersonnel().then((res) => {
            ...
            if (res.data && res.data.length > 0) {
                for (let item of res.data) {
                    if (!compHandle.Position.find(i => i.label === item.Position) && item.Position) {
                        compHandle.Position.push({
                            label: item.Position,
                            value: item.Position
                        })
                    }
    ...
    dels() {
        ...
        deleteModalRef.value.openDeleteModal('确认要删除所选人员吗？',function deleteFun() {
            deleteItem(ids)
    ...
    search() {
        let props = ['PersonName', 'GroupName', 'Position', 'DT', 'Tel']
        compData.tableData  = Search(compData.searchForm.keyWord, props, deepCopy(compData.allData))
    },
    exportData() {
        ExportTable(compData.allData, compData.columns, '人员管理')
    },
})
