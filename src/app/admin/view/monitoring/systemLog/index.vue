...
import {findAllSystemLog} from "@/app/admin/api/systemLog"
...
const compHandle = reactive({
        ...
        findAllSystemLog().then((res) => {
            ...
            if (data && data.length > 0) {
                for (let item of data) {
                    if (!compHandle.filterArr.find(i => i.value === item.RunningState)) {
                        compHandle.filterArr.push({
                            label: item.RunningState,
                            value: item.RunningState
                        })
                    }
                }
            }
    ...
    exportData() {
        ExportTable(compData.allData, compData.columns, '系统日志')
    },
})
