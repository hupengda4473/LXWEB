<template>
    <div class="pageContent" ref="pageContentRef">
        ...
        <!-- 分配角色 -->
        <role ref="roleRef" @refreshTable="compHandle.getTableData()"/>
    </div>
</template>
...
import {getAllUsers, deleteUsers, findMenuByUserID, findRoleByUserID} from '@/app/admin/api/user'
...
const compHandle = reactive({
    ...
    dels() {
        ...
        deleteModalRef.value.openDeleteModal('确认要删除所选用户吗？',function deleteFun() {
            deleteUserData(ids)
            compData.checkedRowKeys = []
        })
    },
    edit(row: any) {
        addModalRef.value.openModal({type: 'edit', itemData: deepCopy(row)})
    },
    add() {
    ...
    search() {
        let props = ['UserName', 'NickName', 'NickName']
        compData.tableData  = Search(compData.searchForm.keyWord, props, deepCopy(compData.allData))
    },
    assignPermissions(row: object) {
        findMenuByUserID(row.UserID).then( res => {
            powerRef.value.drawerOpen( deepCopy(res.data), row.UserID )
        })
    },
    assignRole(row: object) {
        findRoleByUserID(row.UserID).then( res => {
            roleRef.value.modalOpen( deepCopy(res.data), row.UserID )
        })
    },
    exportData() {
        ExportTable(compData.allData, compData.columns, '用户管理')
    },
})

//删除角色
const deleteUserData = (ids : string | number) => {
    compData.loading = true
