                                        ...
                                        <n-grid cols="24" x-gap="30" item-responsive responsive="screen">
                                            ...
                                            <n-grid-item span="24 m:6 l:6">
                                                <n-form-item label="采集时间：" path="keyWord">
                                                    <n-date-picker style="width: 100%" placement="bottom-end" v-model:value="compData.searchForm.time" type="date" :shortcuts="compData.shortcuts" placeholder="请选采集时间"/>
                                                </n-form-item>
                                            </n-grid-item>
                                            <n-grid-item span="24 m:6 l:6">
                                                <n-form-item>
...
import {DeleteWaterQuality, HistoryDataWaterQuality} from "@/app/admin/api/waterQuality"
...
const compHandle = reactive({
    ...
    getTableData() {
        ...
        let params = {
            PageSize: 999999999,
            PageIndex: 1,
            LocationId: compData.searchForm.LocationID || 0,
            CollectionTime: [ new Date(compData.searchForm.time).format("yyyy-MM-dd") ]
        }
        HistoryDataWaterQuality(params).then((res) => {
            ...
            if (data && data.length > 0) {
                for (let item of data) {
                    if (!compHandle.filterArr.find(i => i.value === item.LocationId) && item.LocationId) {
    ...
    exportData() {
        ExportTable(compData.allData, compData.columns, '水质历史数据')
    },
})
