...
import {tableSetting, btnConfig, headwatersTypeList} from '@/app/admin/config/config.js'
...
import {FindWaterSource, DeleteWaterSource} from "@/app/admin/api/waterSource"
...
const compHandle = reactive({
    ...
    getTableData() {
        ...
        FindWaterSource(params).then((res) => {
            ...
            if (data && data.length > 0) {
                for (let item of data) {
                    if (!compHandle.filterArr.find(i => i.value === item.SourceType) && item.SourceType) {
                        compHandle.filterArr.push({
                            label: headwatersTypeList.find(i2 => i2.value == item.SourceType).label,
                            value: item.SourceType
                        })
                    }
                }
            }
            let props = ['SourceName', 'Capacity']
    ...
    exportData() {
        ExportTable(compData.allData, compData.columns, '水源信息管理')
    },
})
