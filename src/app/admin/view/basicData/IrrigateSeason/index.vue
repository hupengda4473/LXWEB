...
import {findAllIrrigateSeason, deleteIrrigateSeason} from "@/app/admin/api/IrrigateSeason"
...
const compHandle = reactive({
    ...
    getTableData() {
        ...
        findAllIrrigateSeason(params).then((res) => {
    ...
    dels() {
        ...
        deleteModalRef.value.openDeleteModal('确认要删除所选灌季吗？',function deleteFun() {
            deleteItem(ids)
    ...
    search() {
        let props = ['SeasonName', 'Round', 'BeginDT', 'EndDT']
        compData.tableData  = Search(compData.searchForm.keyWord, props, deepCopy(compData.allData))
    },
    exportData() {
        ExportTable(compData.allData, compData.columns, '灌季管理')
    },
})
