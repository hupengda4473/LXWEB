        ...
        <n-grid cols="24" x-gap="10" item-responsive responsive="screen">
            <n-grid-item span="24 m:24 l:24">
                <n-space :wrap-item="false">
                    <n-card :segmented="{content: true,footer:true}" header-style="padding:0"
                        ...
                        <template #header>
                            ...
                            >
                                    ...
                                    >
                                        <n-grid cols="24" x-gap="30" item-responsive responsive="screen">
                                            ...
                                            <n-grid-item span="24 m:8 l:6">
                                                <n-form-item>
                                                    <n-space>
                                                        <n-button attr-type="button" @click="compHandle.search"
                                                                  :color="btnConfig.ser">
                                                            <template #icon
                                                                      v-if="btnConfig.showIco && btnConfig.ico.ser">
                                                                <component
                                                                    class="ico"
                                ...
                                <n-tab-pane name="数据操作">
                                    <n-space>
                                        <n-button :color="btnConfig.exp" v-if="compHandle.operation.isExport"
                                                  @click="compHandle.exportData">
                                        ...
                                        <n-button :color="btnConfig.ref" :loading="compData.loading"
                                        ...
                                        <n-popselect v-if="tableSetting.showTableSizeSetting"
                                                     v-model:value="compData.tableSizeValue"
                                                     :options="tableSetting.tableSize" trigger="click">
                                        ...
                                        <n-popover v-if="tableSetting.showTableColumnsSetting" trigger="click"
                                                   placement="bottom">
                                            <template #trigger>
                                            ...
                                            <n-checkbox-group v-model:value="compData.columnsOptionsValue"
                                                ...
                                                <n-space vertical align="start">
                                                    <n-checkbox v-for="item in compData.columnsOptions"
                                                                :key="item.value"
                                                                :value="item.key"
                                                                :label="item.title"
...
import {FindYearReport} from "@/app/admin/api/sluiceGate"
...
const compHandle = reactive({
    ...
    getTableData() {
        ...
        let params = {
            LocationID: compData.searchForm.GroupID || 1,
            StationID: compData.searchForm.deviceId,
            Year: new Date(compData.searchForm.time).format('yyyy'),
        }
        FindYearReport(params).then((res) => {
            let data = res.data
            ...
            data.push({
                DT: "月合计",
                S1: data.reduce((prevValue, row) => prevValue + row.S1, 0),
                S2: data.reduce((prevValue, row) => prevValue + row.S2, 0),
                S3: data.reduce((prevValue, row) => prevValue + row.S3, 0),
                S4: data.reduce((prevValue, row) => prevValue + row.S4, 0),
                S5: data.reduce((prevValue, row) => prevValue + row.S5, 0),
                S6: data.reduce((prevValue, row) => prevValue + row.S6, 0),
                S7: data.reduce((prevValue, row) => prevValue + row.S7, 0),
                S8: data.reduce((prevValue, row) => prevValue + row.S8, 0),
                S9: data.reduce((prevValue, row) => prevValue + row.S9, 0),
                S10: data.reduce((prevValue, row) => prevValue + row.S10, 0),
                S11: data.reduce((prevValue, row) => prevValue + row.S11, 0),
                S12: data.reduce((prevValue, row) => prevValue + row.S12, 0),
            })
            data.push({
                DT: "年合计",
                S1: (total).toFixed(2),
            ...
            compData.sourceColumns = createColumns(compData.allData)
    ...
    exportData() {
        ExportTable(compData.allData, compData.columns, ' 报表管理')
    },
})
