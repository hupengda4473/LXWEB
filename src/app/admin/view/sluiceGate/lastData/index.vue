...
import {FindSluiceDataLast} from "@/app/admin/api/sluiceGate"
...
const compHandle = reactive({
    ...
    getTableData() {
        ...
        FindSluiceDataLast(params).then((res) => {
            let data = res.data
            compData.tableData = data || []
            compData.allData = data || []
            compHandle.tableData = data || []
    ...
    search() {
        let props = ['PGroupName', 'GroupName', 'DT']
        compData.tableData  = Search(compData.searchForm.keyWord, props, deepCopy(compData.allData))
        compHandle.tableData  = Search(compData.searchForm.keyWord, props, deepCopy(compData.allData))
    },
    exportData() {
        ExportTable(compData.allData, compData.columns, '闸门最新数据')
    },
})
