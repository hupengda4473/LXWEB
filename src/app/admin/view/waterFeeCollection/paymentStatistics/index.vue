...
import {DeletePaymentRecord, FindAllPaymentRecord} from "@/app/admin/api/waterFeeCollection"
...
const compHandle = reactive({
        ...
        FindAllPaymentRecord().then((res) => {
            ...
            if (data && data.length > 0) {
                for (let item of data) {
                    if (!compHandle.filterArr.find(i => i.value === item.AssociationID) && item.AssociationID) {
    ...
    search() {
        let props = ['AssociationName', 'Remark', 'PaymentAmount', 'PaymentDT', 'Year']
        compData.tableData  = Search(compData.searchForm.keyWord, props, deepCopy(compData.allData))
    },
    exportData() {
        ExportTable(compData.allData, compData.columns, '缴费记录')
    },
})
